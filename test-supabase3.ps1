...
# Test with a completely fresh random email
$body = @{ email = "fresh$(Get-Random)@outlook.com"; password = 'TestPass123!' } | ConvertTo-Json
try {
    ...
    Write-Host "FRESH EMAIL STATUS: $($r.StatusCode)"
    Write-Host "BODY: $($r.Content.Substring(0, [Math]::Min(300, $r.Content.Length)))"
} catch {
    Write-Host "FRESH EMAIL STATUS: $($_.Exception.Response.StatusCode.value__)"
